<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($sqlNumOfRows < 1){
            $sql2 = $connect->prepare("INSERT INTO buku (isbn, judul, pengarang, abstrak, tanggal_terbit, penerbit)
                                                        VALUES(:isbn,:judul,:pengarang,:abstrak,:tanggal_terbit,:penerbit)");
            ...
            if ($sql2 -> execute($array_execute)){
                $result['message'] = "Berhasil menambah ($judul) !";
            }else{
                $result["error"] = true;
            ...
            $result["error"] = true;
            $result['message'] = "ISBN ($isbn) sudah ada! ";
        }
    }else{
        $result["error"] = true;
